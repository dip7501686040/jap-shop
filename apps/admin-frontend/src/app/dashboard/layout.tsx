...
function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen max-h-screen overflow-hidden bg-gray-100">
      ...
      <button className="fixed top-4 left-4 z-50 md:hidden bg-gray-900 text-white p-2 rounded focus:outline-none" onClick={() => setSidebarOpen((open) => !open)} aria-label="Toggle sidebar">
        {sidebarOpen ? (
      ...
      <div className="flex-1 flex flex-col min-h-screen max-h-screen transition-all duration-300 overflow-hidden">
        ...
        <main className="flex-1 p-4 bg-gray-100 text-black h-full overflow-hidden">{children}</main>
      </div>
    </div>
  )
}
